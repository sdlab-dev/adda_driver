...
int main(int argc, char** argv)
{
  ...
  //for command 8bit
  buf[10] = 0x04; // e7MemIface WRITE COMMAND
  ...
  for(int i = 0; i < 1024; i++){
    buf[i+20] = (argc > 2 ? v : i) & 0x000000FF;
  }

  sdlab_sendrecv((char*)"10.0.0.1", 0x4000, (char*)buf, 1024 + 20, buf, 4);
  printf("%08x\n", ntohl(*(int*)buf));

  x = htonl(CMD_KICK_DRAMWRITE);
